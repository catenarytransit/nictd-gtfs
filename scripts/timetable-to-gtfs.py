...
def build_from_sheet(df: pd.DataFrame, sheet_name: str):
    ...
    default_dir = find_value_to_right(df, "Direction")
    ...
    if meta_row is not None:
        ...
        tripshortname_col = idx_for("trip short name","trip_short_name")
    ...
    for r in range(data_start, df.shape[0]):
        ...
        if tripshortname_col is not None and pd.notna(row.iloc[tripshortname_col]):
            txt=str(row.iloc[tripshortname_col]).strip()
            trip_short_name = txt if txt != "" else None
        ...
        if trip_short_name: trip["trip_short_name"] = trip_short_name
        trips.append(trip)
...
def main():
    ...
    trips_cols = ["route_id","service_id","trip_id","trip_short_name"]
